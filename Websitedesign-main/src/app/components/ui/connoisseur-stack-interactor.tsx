...
export const Component = ({
  ...
  const masterTl = useRef<{ kill?: () => void } | null>(null);

  const createLoop = (index: number) => {
    const item = items[index];
    if (masterTl.current?.kill) masterTl.current.kill();
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full md:w-1/2 flex justify-center items-center mt-16 md:mt-0">
        ...
        <svg
          viewBox="0 0 500 500"
          className="w-[100%] max-w-[500px] h-auto z-10 drop-shadow-xl dark:drop-shadow-[0_0_60px_rgba(0,0,0,0.8)]"
        >
          <defs>
