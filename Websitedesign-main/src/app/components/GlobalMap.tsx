...
export function GlobalMap({ showHeader = true }: { showHeader?: boolean }) {
  ...
  useEffect(() => {
    ...
    const globe = createGlobe(canvasRef.current, {
      ...
      markers: locations.map(l => ({ 
        location: l.coords as [number, number], 
  ...
  }, [locations, focusLocation]); // Re-create if focus changes (optional, but safer for now)
  
  const handleMouseMove = (e: React.MouseEvent) => {
    const rect = e.currentTarget.getBoundingClientRect();
    const x = (e.clientX - rect.left) / rect.width; // 0 to 1
    // Map mouse X to rotation offset (-1 to 1 radians)
    const offset = (x - 0.5) * 2; 
    targetRRef.current = offset;
  };
  ...
  const handleLocationClick = (locId: string) => {
     ...
     if (loc) {
        // Here we could calculate the exact phi needed to center the location
        // For now, we just update the card state
     }
  };

  return (
    <div className={`relative w-full max-w-5xl mx-auto ${showHeader ? 'py-20 px-6' : ''}`}>
      {showHeader && (
      ...
      <div className={`relative aspect-[16/9] w-full bg-[#0F0F0F] rounded-3xl border border-white/5 overflow-hidden shadow-2xl flex items-center justify-center ${!showHeader ? 'bg-transparent border-none shadow-none aspect-square' : ''}`}>
        ...
        <div className="absolute bottom-6 left-0 right-0 z-20 flex justify-center gap-2 flex-wrap px-4 pointer-events-none">
             {locations.map((loc) => (
                 ...
                 >
                    <span className={`w-1.5 h-1.5 rounded-full ${loc.operational ? 'bg-green-500 animate-pulse' : 'bg-amber-500'}`} />
                    {loc.name.split(',')[0]}
                 </button>
             ))}
        </div>
        ...
        <AnimatePresence>
          {activeLoc && (
            ...
            >
              <button 
                onClick={(e) => { e.stopPropagation(); setFocusLocation(null); }}
              ...
              <div className="flex items-start gap-3 mb-5">
                ...
                <div>
                  <h3 className="text-base font-bold text-white leading-tight">{activeLoc.name}</h3>
              ...
              {activeLoc.operational && (
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-white font-mono">{activeLoc.ping}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2 text-white/60">
                    ...
                    <span className="text-white font-medium truncate ml-2">{activeLoc.cpu}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2 text-white/60">
                    ...
                    <span className="text-white font-medium">{activeLoc.ram}</span>
                  </div>
                </div>
              )}
            </motion.div>
